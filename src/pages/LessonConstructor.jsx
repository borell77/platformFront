...
export default function LessonConstructor() {
  ...
  // Загружаем список заданий для выбора
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await api.get('/tasks?subject=math'); // можно расширить на другие предметы
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await api.post(`/lessons/group/${groupId}`, requestData);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <header className="sticky top-0 z-10 backdrop-blur-sm bg-white/80 border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold">Конструктор урока</h1>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            {blocks.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-300">
                <p className="text-gray-600">Добавьте первый блок, чтобы начать</p>
              </div>
            ) : (
          ...
          <div className="flex gap-3">
            ...
            >
              {loading ? 'Сохранение...' : 'Сохранить урок'}
            </button>
          </div>
        </form>
      </main>
...
// Компонент редактирования одного блока
